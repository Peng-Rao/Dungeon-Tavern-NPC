...
struct TechniqueRef {
  ...
  void init(const char *_id, std::vector<PipelineAndTexturesDefs> _PT, int _Ntextures,
...
public:
  ...
  int init(BaseProject *_BP, int _Npasses, std::vector<VertexDescriptorRef> &VDRs,
  ...
  Collider *CreateColliderRecursive(ColliderDef &def, std::vector<Collider *> &GlobalList,
                                    ColliderShow &Show, bool parentVisible);
  void setColliderStroke(Collider *c, glm::vec4 color);
};

#ifdef SCENE_IMPLEMENTATION

void TechniqueRef::init(const char *_id, std::vector<PipelineAndTexturesDefs> _PT, int _Ntextures,
                        VertexDescriptor *_VD) {
...
int Scene::init(BaseProject *_BP, int _Npasses, std::vector<VertexDescriptorRef> &VDRs,
  ...
  for (int i = 0; i < PRs.size(); i++) {
    if (PRs[i].PT.size() != Npasses) {
      std::cout << "Scene Error: the number of pipelines for technique " << *PRs[i].id
                << " does not correspond to the number of passes : " << PRs[i].PT.size()
                << " != " << Npasses << "\n";
      exit(0);
    }
  ...
  for (int k = 0; k < ModelCount; k++) {
    ...
    if (MT[0] == 'A') {
      ...
      M[k]->init(BP, VDIds[VDN], ms[k]["model"],
    ...
    for (int j = 0; j < TI[k].InstanceCount; j++) {
      ...
      if (TMjson.is_null()) {
        ...
        if (!Tr_REjson.is_null()) {
          trR = glm::rotate(glm::mat4(1.0f), glm::radians((float)Tr_REjson[1]),
                            glm::vec3(0.0f, 1.0f, 0.0f)) *
                glm::rotate(glm::mat4(1.0f), glm::radians((float)Tr_REjson[0]),
                            glm::vec3(1.0f, 0.0f, 0.0f)) *
                glm::rotate(glm::mat4(1.0f), glm::radians((float)Tr_REjson[2]),
                            glm::vec3(0.0f, 0.0f, 1.0f));
          manualPos = true;
        } else {
          nlohmann::json Tr_RQjson = is[j]["quaternion"];
          if (!Tr_RQjson.is_null()) {
            glm::quat trQ = glm::quat(Tr_RQjson[0], Tr_RQjson[1], Tr_RQjson[2], Tr_RQjson[3]);
            trR = glm::mat4(trQ);
            manualPos = true;
          }
        }
        ...
        if (manualPos) {
          TI[k].I[j].Wm =
              glm::translate(glm::mat4(1.0f), trT) * trR * glm::scale(glm::mat4(1.0f), trS);
        } else {
        ...
        TI[k].I[j].Wm =
            glm::mat4(TMj[0], TMj[4], TMj[8], TMj[12], TMj[1], TMj[5], TMj[9], TMj[13], TMj[2],
      ...
      TI[k].I[j].TIp = &TI[k];
      TI[k].I[j].D = (std::vector<DescriptorSetLayout *> **)calloc(
          sizeof(std::vector<DescriptorSetLayout *> *), Npasses);
...
void Scene::pipelinesAndDescriptorSetsInit() {
  ...
  for (int i = 0; i < InstanceCount; i++) {
    // std::cout << "I: " << i << ", NTx: " << I[i]->NTx << ", NDs: " << I[i]->NDs << ", nPasses: "
    // << Npasses << "\n";
      ...
      for (int j = 0; j < I[i]->NDs[ipas]; j++) {
        ...
        for (int kt = 0; kt < ntxs; kt++) {
          if (Tr->PT[ipas].texDefs[j][kt].fromInstance) {
            Tids[kt] = T[I[i]->Tid[Tr->PT[ipas].texDefs[j][kt].pos]]->getViewAndSampler();
            // std::cout << "Getting " << Tids[kt].sampler << " " << Tids[kt].imageView << " " <<
            // Tids[kt].imageLayout << " from insance for: i" << i << " p" << ipas << " d" << j << "
            // t" << kt << "\n";
          } else {
            Tids[kt] = Tr->PT[ipas].texDefs[j][kt].info;
            // std::cout << "Ext. tex. :" << &Tids[kt] << "\n";
            // std::cout << "Getting " << Tids[kt].sampler << " " << Tids[kt].imageView << " " <<
            // Tids[kt].imageLayout << " from technique for: i" << i << " p" << ipas << " d" << j <<
            // " t" << kt << "\n"; 						Tids[kt] =
            // T[0]->getViewAndSampler();
          }
        }

        I[i]->DS[ipas][j] = new DescriptorSet();
        // std::cout << "Allocating DS for DSL: " << (*I[i]->D[ipas])[j] << ", with " << Tids.size()
        // << " textures\n";
  ...
  for (int k = 0; k < TechniqueInstanceCount; k++) {
    ...
    for (int i = 0; i < TI[k].InstanceCount; i++) {
      ...
      if (P != nullptr) {
        ...
        // std::cout << "Draw Call\n";
        vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(M[TI[k].I[i].Mid]->indices.size()), 1,
                         0, 0, 0);
      }
    }
  }
...
// This function forces a refresh of the visualized colliders (rebuild mesh and re-record command
// buffer)
...
void Scene::ParseColliderRecursive(nlohmann::json &node, ColliderDef &def, Model **M,
                                   int modelIndex) {
  ...
  if (node.is_string()) {
    ...
    if (def.type == "AABB") {
      ...
      std::cout << "Scene Error: shortcut for collider type: " << def.type << " not supported "
                << "\n";
      exit(0);
    }
  }
  // Complete collider specification
    ...
    if (node.contains("params")) {
      for (auto &el : node["params"])
        def.params.push_back(el);
    }
...
Collider *Scene::CreateColliderRecursive(ColliderDef &def, std::vector<Collider *> &GlobalList,
                                         ColliderShow &Show, bool parentVisible) {
  ...
  if (def.type == "AABB") {
    if (def.params.size() < 6) {
      std::cout << "Scene Error: wrong number of parameters for collider type: " << def.type
                << "\n";
      exit(0);
    }
    newCol->initAABB(def.params[0], def.params[1], def.params[2], def.params[3], def.params[4],
                     def.params[5]);
  } else if (def.type == "OOBB") {
    if (def.params.size() < 6) {
      std::cout << "Scene Error: wrong number of parameters for collider type: " << def.type
    ...
    newCol->initOOBB(def.params[0], def.params[1], def.params[2], def.params[3], def.params[4],
                     def.params[5]);
    ...
    if (def.children.empty()) {
      std::cout << "Scene Error: collider type: " << def.type << " must have children \n";
      exit(0);
    }
    ...
    std::cout << "Scene Error: collider type: " << def.type << " not supported " << "\n";
    exit(0);
  }
