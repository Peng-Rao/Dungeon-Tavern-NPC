...
struct AnimTrack {
  ...
  void getSampleTransforms(glm::vec3 &T, glm::quat &Q, glm::vec3 &S, float t, int sf, int ef,
                           bool loop);
  glm::mat4 Sample(float t, int sf, int ef, bool loop);
  glm::mat4 Blend(float bf, float tinA, int sfA, int efA, float tinB, int sfB, int efB,
                  AnimTrack *B = nullptr);
};
...
void AnimTrack::getSampleTransforms(glm::vec3 &T, glm::quat &Q, glm::vec3 &S, float tin, int sf,
                                    int ef, bool loop) {
  glm::vec3 T0, T1;
  glm::quat Q0, Q1;
  glm::vec3 S0, S1;
  ...
  float lastT = (ef >= nKeyFrames) ? 2 * Frames[nKeyFrames - 1].time - Frames[nKeyFrames - 2].time
                                   : Frames[ef].time;
  ...
  // std::cout << "Found: " << srcl << " " << srcr << " " << Frames[srcl].time << " " << t << " " <<
  // ((srcl + 1 < ef) ? Frames[srcl + 1].time : lastT) << "\n";
...
glm::mat4 AnimTrack::Sample(float tin, int sf = 0, int ef = -1, bool loop = false) {
  ...
  out = glm::translate(glm::mat4(1), T) * glm::mat4(Q) * glm::scale(glm::mat4(1), S);

  return out;
}

glm::mat4 AnimTrack::Blend(float bf, float tinA, int sfA, int efA, float tinB, int sfB, int efB,
                           AnimTrack *B) {
  if (B == nullptr) {
    B = this;
  }
...
glm::mat4 AnimBlender::Sample(AnimTrack *AT, AnimTrack *AT2) {
  ...
  if (blending) {
    return AT->Blend(1.0f - blendPos / blendTime, segments[cur].t, segments[cur].st,
                     segments[cur].en, segments[prev].t, segments[prev].st, segments[prev].en, AT2);
  } else {
...
void Animations::init(AssetFile &A) {
  ...
  for (const auto &anim : model->animations) {
    std::cout << " Anim. Name:" << anim.name << " Channels: " << anim.channels.size()
    ...
    for (const auto &chan : anim.channels) {
      //			std::cout << chan.target_path << " Node: " << chan.target_node << "
      // Sampler: " << chan.sampler << " input: " << anim.samplers[chan.sampler].input << " output:
      // "
      //<< anim.samplers[chan.sampler].output <<"\n";
      ...
      if (Time == nullptr) {
        ...
        const float *inVals = reinterpret_cast<const float *>(
            &(model->buffers[inView.buffer].data[inAccessor.byteOffset + inView.byteOffset]));
      ...
      const float *outVals = reinterpret_cast<const float *>(
          &(model->buffers[outView.buffer].data[outAccessor.byteOffset + outView.byteOffset]));
    ...
    for (auto tninf : nodeIds) {
      ...
      for (int kf = 0; kf < nKeyFrames; kf++) {
        ...
        if (Rot[targetNode] != nullptr) {
          Q = glm::quat(Rot[targetNode][3], Rot[targetNode][0], Rot[targetNode][1],
                        Rot[targetNode][2]);
          Rot[targetNode] += 4;
        }
...
void SkeletalAnimation::init(Animations *_anims, int _NAnims, std::string BaseTrackName,
                             int SkinId) {
  anims = _anims;
  NAnims = _NAnims;
  ...
  for (int naic = 0; naic < NAnims; naic++) {
    ...
    if (naic == 0) {
      ...
      if (model->skins[SkinId].joints.size() != skin->joints.size()) {
        std::cout << "Error! Animation " << naic
                  << " has a different number of joints compared to Animation 0\n"
    ...
    if (naic == 0) {
      for (int i = 0; i < skin->joints.size(); i++) {
        ...
        if (at != nullptr) {
          ...
          Model::getGLTFnodeTransforms(&model->nodes[targetNode], T, S, Q);
          BaseTMs[targetNode] =
              glm::translate(glm::mat4(1), T) * glm::mat4(Q) * glm::scale(glm::mat4(1), S);
      ...
      for (int i = 0; i < skin->joints.size(); i++) {
        ...
        if (at != nullptr) {
          ...
          if (ATsNodeId[atsCorrI] == targetNode) {
            ...
            std::cout << "Error! Animation " << naic
                      << " doest not match Animation 0 skin structure\n";
            exit(0);
          }
        } else {
          glm::vec3 T;
  ...
  for (int mel = 0; mel < NTMs; mel++) {
    ...
    IBMs[skin->joints[mel]] = glm::mat4(s[0], s[1], s[2], s[3], s[4], s[5], s[6], s[7], s[8], s[9],
...
void SkeletalAnimation::Sample(AnimBlender &AB) {
  ...
  for (int i = 0; i < NTMs; i++) {
    TMs[i] = TMs[i] * IBMs[i];
    // std::cout << i << " <> " << model->nodes[i].name << ": " << (-IBMs[i][3][0]) << " " <<
    // (-IBMs[i][3][1]) << " " << (-IBMs[i][3][2]) << "\n";
    /*std::cout << i << " = \n";
    for(int mi = 0; mi<16; mi++) {
