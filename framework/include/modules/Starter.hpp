...
class Model {
	...
	VertexDescriptor *getVD() {return VD;}
};
...
struct Texture {
	...
	void createTexture(int texWidth, int texHeight, int texChannels, int texChannelSize, std::vector<void *> pixels, VkFormat Fmt = VK_FORMAT_R8G8B8A8_SRGB);
	...
	void initCubic(BaseProject *bp, std::vector<std::string>, VkFormat Fmt = VK_FORMAT_R8G8B8A8_SRGB, bool initSampler = true);
	void initPixels(BaseProject *bp, int texWidth, int texHeight, int texChannels, int texChannelSize, std::vector<void *> pixels, VkFormat Fmt = VK_FORMAT_R8G8B8A8_SRGB, bool initSampler = true);
...
enum StockAttchmentsConfiguration {AT_SURFACE_AA_DEPTH, AT_ONE_COLOR_AND_DEPTH, AT_DEPTH_ONLY, AT_SURFACE_NOAA_DEPTH, AT_GBUFFER, AT_NO_ATTCHMENTS};
...
*/		}
		if(!raytraceOk) {
			std::cout << "Error! not all the required extensions are available. Raytracing cannot be done\n";
			exit(0);
		}
...
void BaseProject::createDescriptorPool() {
	...
	if(DPSZs.uniformBlocksInPool > 0) {
		poolSizes.emplace_back(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, static_cast<uint32_t>(DPSZs.uniformBlocksInPool * swapChainImages.size()));
	}

	if(DPSZs.texturesInPool > 0) {
		poolSizes.emplace_back(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, static_cast<uint32_t>(DPSZs.texturesInPool * swapChainImages.size()));
	}
	
	if(DPSZs.samplersInPool > 0) {
		poolSizes.emplace_back(VK_DESCRIPTOR_TYPE_SAMPLER, static_cast<uint32_t>(DPSZs.samplersInPool * swapChainImages.size()));
	}
	
	if(DPSZs.sampledImagesInPool > 0) {
		poolSizes.emplace_back(VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, static_cast<uint32_t>(DPSZs.sampledImagesInPool * swapChainImages.size()));
	}
...
void TextureSampler::init(BaseProject *lBP,
							 VkFilter magFilter,
							 VkFilter minFilter,
...
void Texture::createTexture(int texWidth, int texHeight, int texChannels, int texChannelSize, std::vector<void *> pixels, VkFormat Fmt) {
...
void Texture::initCubic(BaseProject *bp, std::vector<std::string>files, VkFormat Fmt, bool initSampler) {
	if(files.size() != 6) {
...
void Texture::initPixels(BaseProject *bp, int texWidth, int texHeight, int texChannels, int texChannelSize, std::vector<void *> pixels, VkFormat Fmt, bool initSampler) {
	BP = bp;
	imgs = pixels.size();
...
/* SI DEVE AGGIUNGERE UNA VARIABILE per initSampler, ed inizializzarlo nella chiamata createResources */
...
void RenderPass::begin(VkCommandBuffer commandBuffer, int currentImage) {
	...
	if(connectedToSwapchain) {
		renderPassInfo.renderArea.extent = {(uint32_t)(width < BP->swapChainExtent.width ?
													   width : BP->swapChainExtent.width),
											(uint32_t)(height < BP->swapChainExtent.height ?
													   height : BP->swapChainExtent.height)};
	} else {
