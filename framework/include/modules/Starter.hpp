...
struct VertexDescriptor {
  ...
  void init(BaseProject *bp, std::vector<VertexBindingDescriptorElement> B,
...
public:
  ...
  void initFromAsset(BaseProject *bp, VertexDescriptor *VD, AssetFile *AF, std::string AN,
                     int Mid = 0, std::string NN = "");
  void initMesh(BaseProject *bp, VertexDescriptor *VD, bool printDebug = true);
  void cleanup();
  void bind(VkCommandBuffer commandBuffer);

  VertexDescriptor *getVD() {
    return VD;
  }
};
...
struct TextureSampler {
  ...
  void init(BaseProject *lBP, VkFilter magFilter = VK_FILTER_LINEAR,
            VkFilter minFilter = VK_FILTER_LINEAR,
            VkSamplerAddressMode addressModeU = VK_SAMPLER_ADDRESS_MODE_REPEAT,
            VkSamplerAddressMode addressModeV = VK_SAMPLER_ADDRESS_MODE_REPEAT,
            VkSamplerMipmapMode mipmapMode = VK_SAMPLER_MIPMAP_MODE_LINEAR,
            VkBool32 anisotropyEnable = VK_TRUE, float maxAnisotropy = 16, float maxLod = 128);
  void cleanup();
...
struct Texture {
  ...
  void createTexture(int texWidth, int texHeight, int texChannels, int texChannelSize,
                     std::vector<void *> pixels, VkFormat Fmt = VK_FORMAT_R8G8B8A8_SRGB);
  ...
  void setSampler(TextureSampler *smp);

  void init(BaseProject *bp, std::string file, VkFormat Fmt = VK_FORMAT_R8G8B8A8_SRGB,
            bool initSampler = true);
  void initCubic(BaseProject *bp, std::vector<std::string>, VkFormat Fmt = VK_FORMAT_R8G8B8A8_SRGB,
                 bool initSampler = true);
  void initPixels(BaseProject *bp, int texWidth, int texHeight, int texChannels, int texChannelSize,
                  std::vector<void *> pixels, VkFormat Fmt = VK_FORMAT_R8G8B8A8_SRGB,
                  bool initSampler = true);
...
enum StockAttchmentsConfiguration {
  AT_SURFACE_AA_DEPTH,
  AT_ONE_COLOR_AND_DEPTH,
  AT_DEPTH_ONLY,
  AT_SURFACE_NOAA_DEPTH,
  AT_GBUFFER,
  AT_NO_ATTCHMENTS
};

enum StockAttchmentsDependencies {
  ATDEP_SIMPLE,
  ATDEP_SURFACE_ONLY,
  ATDEP_DEPTH_TRANS,
  ATDEP_NO_DEP
...
struct RenderPass {
  ...
  void init(BaseProject *bp, int w = -1, int h = -1, int _count = -1,
            std::vector<AttachmentProperties> *p = nullptr,
            std::vector<VkSubpassDependency> *d = nullptr, bool initSampler = false);
  void create();
  void begin(VkCommandBuffer commandBuffer, int currentImage);
  void end(VkCommandBuffer commandBuffer);
  void cleanup();
  void destroy();
  static std::vector<AttachmentProperties> *
  getStandardAttchmentsProperties(StockAttchmentsConfiguration cfg, BaseProject *BP);
...
struct Pipeline {
  ...
  void init(BaseProject *bp, VertexDescriptor *vd, const std::string &VertShader,
            const std::string &FragShader, std::vector<DescriptorSetLayout *> d,
            std::vector<VkPushConstantRange> pk = {});
...
struct DescriptorSet {
  ...
  void init(BaseProject *bp, DescriptorSetLayout *L, std::vector<VkDescriptorImageInfo> VaSs);
  void cleanup();
...
struct PoolSizes {
  ...
  int samplersInPool = 0;
  int sampledImagesInPool = 0;
  int setsInPool = 0;
};
...
enum NamedCommandBuffersStates {
  NCBS_SUBMITTED,
  NCBS_IN_CREATION,
  NCBS_IN_USE,
  NCBS_TO_DELETE,
...
public:
  ...
  void run(bool _includeRaytracing = false);
...
protected:
  ...
  void createImage(uint32_t width, uint32_t height, uint32_t mipLevels, int imgCount,
                   VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling,
                   VkImageUsageFlags usage, VkImageCreateFlags cflags,
  ...
  void transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout,
                             VkImageLayout newLayout, uint32_t mipLevels, int layersCount);
...
public:
  void submitCommandBuffer(std::string name, int order, pNCBfunc populateNewCommandBuffer,
                           void *params, pNCBfree onErase = nullptr);
...
private:
  ...
  vks_initializers_commandBufferAllocateInfo(VkCommandPool commandPool, VkCommandBufferLevel level,
                                             uint32_t bufferCount);
  VkCommandBuffer vulkanDevice_createCommandBuffer(VkCommandBufferLevel level, VkCommandPool pool,
                                                   bool begin);
  ...
  void vulkanDevice_flushCommandBuffer(VkCommandBuffer commandBuffer, VkQueue queue,
                                       VkCommandPool pool, bool freeCmd);
  ...
  void vks_tools_insertImageMemoryBarrier(VkCommandBuffer cmdbuffer, VkImage image,
...
void BaseProject::run(bool _includeRaytracing) {
  ...
  includeRaytracing = _includeRaytracing;
...
void BaseProject::populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT &createInfo) {
  createInfo = {};
...
void BaseProject::pickPhysicalDevice() {
  ...
  for (const auto &device : devices) {
    ...
    std::vector<const char *> ExtensionsForRaytracing = {
    ...
    bool raytraceOk = true;
    if (includeRaytracing) {
      for (auto e : ExtensionsForRaytracing) {
        std::cout << e << "\n";
        if (checkIfItHasDeviceExtension(device, e)) {
    ...
    */		}
    if (!raytraceOk) {
      std::cout
          << "Error! not all the required extensions are available. Raytracing cannot be done\n";
      exit(0);
    }
...
bool BaseProject::isDeviceSuitable(VkPhysicalDevice device, deviceReport &devRep) {
  ...
  if (devRep.extensionsSupported) {
    ...
    devRep.swapChainAdequate =
        !devRep.swapChainPresentModeSupport && !devRep.swapChainPresentModeSupport;
  }
...
VkSurfaceFormatKHR
BaseProject::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR> &availableFormats) {
...
VkPresentModeKHR
BaseProject::chooseSwapPresentMode(const std::vector<VkPresentModeKHR> &availablePresentModes) {
...
void BaseProject::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, int imgCount,
...
void BaseProject::generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth,
...
void BaseProject::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout,
...
void BaseProject::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height,
                                    int layerCount) {
  VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
uint32_t BaseProject::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
...
void BaseProject::createDescriptorPool() {
  ...
  if (DPSZs.uniformBlocksInPool > 0) {
    poolSizes.emplace_back(
        VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
        static_cast<uint32_t>(DPSZs.uniformBlocksInPool * swapChainImages.size()));
  }

  if (DPSZs.texturesInPool > 0) {
    poolSizes.emplace_back(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
                           static_cast<uint32_t>(DPSZs.texturesInPool * swapChainImages.size()));
  }

  if (DPSZs.samplersInPool > 0) {
    poolSizes.emplace_back(VK_DESCRIPTOR_TYPE_SAMPLER,
                           static_cast<uint32_t>(DPSZs.samplersInPool * swapChainImages.size()));
  }

  if (DPSZs.sampledImagesInPool > 0) {
    poolSizes.emplace_back(
        VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,
        static_cast<uint32_t>(DPSZs.sampledImagesInPool * swapChainImages.size()));
  }
...
void BaseProject::submitCommandBuffer(std::string name, int order,
                                      pNCBfunc populateNewCommandBuffer, void *params,
                                      pNCBfree onErase) {
  int sz = swapChainImageViews.size();

  NamedCommandBuffer *nncb = new NamedCommandBuffer{
      name, order, {}, populateNewCommandBuffer, onErase, params, NCBS_SUBMITTED, {}};
  nncb->cb.resize(sz);
  ...
  if (found != namedCommandBuffers.end()) {
    ...
    // std::cout << "Existing command buffer '" << name << "', old size: " <<
    // found->second.old.size() << "\n";
  } else {
...
void BaseProject::clearNamedCommandBufferForImage(NamedCommandBuffer *ncb, int img) {
  if (ncb->inQueue[img]) {
    vkFreeCommandBuffers(device, commandPool, 1, ncb->cb[img]);
    free(ncb->cb[img]);
    ...
    if (v.second.current != nullptr) {
      for (int i = 0; i < sz; i++) {
        if (v.second.current->inQueue[i]) {
          vkFreeCommandBuffers(device, commandPool, 1, v.second.current->cb[i]);
...
void BaseProject::createSyncObjects() {
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    ...
    if (result1 != VK_SUCCESS || result3 != VK_SUCCESS) {
...
void BaseProject::getSixAxis(float &deltaT, glm::vec3 &m, glm::vec3 &r, bool &fire) {
...
void BaseProject::printVec4(const char *Name, glm::vec4 v) {
  std::cout << "glm::vec4 " << Name << " = glm::vec4(" << v[0] << ", " << v[1] << ", " << v[2]
            << ", " << v[3] << ");\n";
}
...
void BaseProject::printQuat(const char *Name, glm::quat q) {
  std::cout << "glm::vec3 " << Name << " = glm::vec3(" << q[0] << ", " << q[1] << ", " << q[2]
            << ", " << q[3] << ");\n";
}
...
VkCommandBuffer BaseProject::vulkanDevice_createCommandBuffer(VkCommandBufferLevel level,
                                                              VkCommandPool pool, bool begin) {
  VkResult result;

  VkCommandBufferAllocateInfo cmdBufAllocateInfo =
      vks_initializers_commandBufferAllocateInfo(pool, level, 1);
...
void BaseProject::vulkanDevice_flushCommandBuffer(VkCommandBuffer commandBuffer, VkQueue queue,
                                                  VkCommandPool pool, bool freeCmd) {
  VkResult result;
...
void BaseProject::saveScreenshot(const char *filename, int currentBuffer) {
  ...
  VkCommandBuffer copyCmd =
      vulkanDevice_createCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, commandPool, true);

  // Transition destination image to transfer destination layout
  vks_tools_insertImageMemoryBarrier(
      copyCmd, dstImage, 0, VK_ACCESS_TRANSFER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
  ...
  vks_tools_insertImageMemoryBarrier(
      copyCmd, srcImage, VK_ACCESS_MEMORY_READ_BIT, VK_ACCESS_TRANSFER_READ_BIT,
  ...
  vks_tools_insertImageMemoryBarrier(
      copyCmd, dstImage, VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_MEMORY_READ_BIT,
  ...
  if (!supportsBlit) {
    ...
    colorSwizzle =
        (std::find(formatsBGR.begin(), formatsBGR.end(), swapChainImageFormat) != formatsBGR.end());
  }
...
void VertexDescriptor::init(BaseProject *bp, std::vector<VertexBindingDescriptorElement> B,
                            std::vector<VertexDescriptorElement> E) {
  BP = bp;
  Bindings = B;
  Layout = E;

  Position.hasIt = false;
  Position.offset = 0;
  Pos2D.hasIt = false;
  Position.offset = 0;
  Normal.hasIt = false;
  ...
  Tangent.hasIt = false;
  Tangent.offset = 0;
  JointWeight.hasIt = false;
  JointWeight.offset = 0;
  JointIndex.hasIt = false;
  JointIndex.offset = 0;

  if (B.size() <=
      1) { // for now, read models only with every vertex information in a single binding
...
void AssetFile::initOBJ(std::string file) {
  ...
  std::cout << "Loading Asset File: " << file
            << "[OBJ] - mat. path: " << (matpath == nullptr ? "<<NOPATH>>" : matpath) << "\n";
  if (!tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, file.c_str(), matpath)) {
  ...
  for (const auto &mesh : model.meshes) {
    ...
    for (const auto &primitive : mesh.primitives) {
      if (primitive.indices < 0) {
        ...
        std::cout << "Primitive: " << PrimCount << ", Material: " << primitive.material << " -> "
...
void Model::makeOBJMesh(const tinyobj::shape_t *M, const tinyobj::attrib_t *A) {
  ...
  for (const auto &index : M->mesh.indices) {
    ...
    if (VD->Position.hasIt) {
      glm::vec3 pos = {A->vertices[3 * index.vertex_index + 0],
    ...
    if (VD->Color.hasIt) {
      glm::vec3 color = {A->colors[3 * index.vertex_index + 0],
    ...
    if (VD->UV.hasIt) {
      glm::vec2 texCoord = {A->texcoords[2 * index.texcoord_index + 0],
    ...
    if (VD->Normal.hasIt) {
      glm::vec3 norm = {A->normals[3 * index.normal_index + 0],
...
void Model::makeGLTFMesh(tinygltf::Model *M, const tinygltf::Primitive *Prm) {
  ...
  if (pIt != Prm->attributes.end()) {
    ...
    bufferPos = reinterpret_cast<const float *>(
        &(M->buffers[posView.buffer].data[posAccessor.byteOffset + posView.byteOffset]));
    meshHasPos = true;
  ...
  if (nIt != Prm->attributes.end()) {
    ...
    bufferNormals = reinterpret_cast<const float *>(
        &(M->buffers[normView.buffer].data[normAccessor.byteOffset + normView.byteOffset]));
    meshHasNorm = true;
  ...
  if (tIt != Prm->attributes.end()) {
    ...
    bufferTangents = reinterpret_cast<const float *>(
        &(M->buffers[tanView.buffer].data[tanAccessor.byteOffset + tanView.byteOffset]));
    meshHasTan = true;
  ...
  if (uIt != Prm->attributes.end()) {
    ...
    bufferTexCoords = reinterpret_cast<const float *>(
        &(M->buffers[uvView.buffer].data[uvAccessor.byteOffset + uvView.byteOffset]));
    meshHasUV = true;
  ...
  if (iIt != Prm->attributes.end()) {
    ...
    bufferJointIndex = reinterpret_cast<const glm::u8 *>(
        &(M->buffers[jointView.buffer].data[jointAccessor.byteOffset + jointView.byteOffset]));
    meshHasJointIndex = true;
    cntJointIndex = jointAccessor.count;
    if (cntJointIndex > cntTot)
      cntTot = cntJointIndex;
  } else {
  ...
  if (wIt != Prm->attributes.end()) {
    ...
    bufferJointWeight = reinterpret_cast<const float *>(&(
        M->buffers[weightsView.buffer].data[weightsAccessor.byteOffset + weightsView.byteOffset]));
    meshHasJointWeight = true;
    cntJointWeight = weightsAccessor.count;
    if (cntJointWeight > cntTot)
      cntTot = cntJointWeight;
  } else {
  ...
  for (int i = 0; i < cntTot; i++) {
    ...
    if ((i < cntPos) && meshHasPos && VD->Position.hasIt) {
      glm::vec3 pos = {bufferPos[3 * i + 0], bufferPos[3 * i + 1], bufferPos[3 * i + 2]};
    ...
    if ((i < cntNorm) && meshHasNorm && VD->Normal.hasIt) {
      glm::vec3 normal = {bufferNormals[3 * i + 0], bufferNormals[3 * i + 1],
                          bufferNormals[3 * i + 2]};
    ...
    if ((i < cntTan) && meshHasTan && VD->Tangent.hasIt) {
      glm::vec4 tangent = {bufferTangents[4 * i + 0], bufferTangents[4 * i + 1],
                           bufferTangents[4 * i + 2], bufferTangents[4 * i + 3]};
    ...
    if ((i < cntUV) && meshHasUV && VD->UV.hasIt) {
      glm::vec2 texCoord = {bufferTexCoords[2 * i + 0], bufferTexCoords[2 * i + 1]};
    ...
    if ((i < cntJointIndex) && meshHasJointIndex && VD->JointIndex.hasIt) {
      glm::uvec4 jointIndex = {
          (glm::uint)bufferJointIndex[4 * i + 0], (glm::uint)bufferJointIndex[4 * i + 1],
          (glm::uint)bufferJointIndex[4 * i + 2], (glm::uint)bufferJointIndex[4 * i + 3]};
      // std::cout << jointIndex.x << " " << jointIndex.y << " " << jointIndex.z << " " <<
      // jointIndex.w << "\t\t";
    ...
    if ((i < cntJointWeight) && meshHasJointWeight && VD->JointWeight.hasIt) {
      glm::vec4 jointWeight = {bufferJointWeight[4 * i + 0], bufferJointWeight[4 * i + 1],
                               bufferJointWeight[4 * i + 2], bufferJointWeight[4 * i + 3]};
      // std::cout << bufferJointWeight[4 * i + 0] << " " << bufferJointWeight[4 * i + 1] << " " <<
      // bufferJointWeight[4 * i + 2] << " " << 				bufferJointWeight[4 * i + 3] << "\n";
  ...
  case TINYGLTF_PARAMETER_TYPE_UNSIGNED_SHORT: {
    const uint16_t *bufferIndex = reinterpret_cast<const uint16_t *>(
  ...
  case TINYGLTF_PARAMETER_TYPE_UNSIGNED_INT: {
    const uint32_t *bufferIndex = reinterpret_cast<const uint32_t *>(
        &(buffer.data[accessor.byteOffset + bufferView.byteOffset]));
    for (int i = 0; i < accessor.count; i++) {
      indices.push_back(bufferIndex[i]);
    }
...
void Model::getGLTFnodeTransforms(const tinygltf::Node *N, glm::vec3 &T, glm::vec3 &S,
                                  glm::quat &Q) {
  if (N->translation.size() > 0) {
    // std::cout << "node " << i << " has T\n";
    T = glm::vec3(N->translation[0], N->translation[1], N->translation[2]);
  } else {
    T = glm::vec3(0);
  }
  if (N->rotation.size() > 0) {
    // std::cout << "node " << i << " has Q\n";
    Q = glm::quat(N->rotation[3], N->rotation[0], N->rotation[1], N->rotation[2]);
  } else {
    Q = glm::quat(1.0f, 0.0f, 0.0f, 0.0f);
  }
  if (N->scale.size() > 0) {
    // std::cout << "node " << i << " has S\n";
    S = glm::vec3(N->scale[0], N->scale[1], N->scale[2]);
  } else {
    S = glm::vec3(1);
  }
}
...
void Model::loadModelGLTF(std::string file, bool encoded) {
  ...
  if (encoded) {
    ...
    plusaes::decrypt_cbc((unsigned char *)modelString.data(), modelString.size(), &key[0],
                         key.size(), &iv, &decrypted[0], decrypted.size(), &padded_size);
    ...
    if (!loader.LoadASCIIFromString(&model, &warn, &err, reinterpret_cast<const char *>(decomp),
                                    size, "/")) {
      throw std::runtime_error(warn + err);
    }
  } else {
  ...
  std::cout << (encoded ? "[MGCG]" : "[GLTF]")
            << " Vertices: " << (vertices.size() / VD->Bindings[0].stride)
...
void Model::initFromAsset(BaseProject *bp, VertexDescriptor *vd, AssetFile *AF, std::string AN,
                          int Mid, std::string NN) {
  BP = bp;
  VD = vd;
  Wm = glm::mat4(1);
...
void TextureSampler::init(BaseProject *lBP, VkFilter magFilter, VkFilter minFilter,
                          VkSamplerAddressMode addressModeU, VkSamplerAddressMode addressModeV,
                          VkSamplerMipmapMode mipmapMode, VkBool32 anisotropyEnable,
  ...
  VkResult result = vkCreateSampler(BP->device, &samplerInfo, nullptr, &textureSampler);
  if (result != VK_SUCCESS) {
    PrintVkError(result);
...
VkDescriptorImageInfo TextureSampler::getViewAndSampler() {
  return {textureSampler, nullptr, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
}
...
void Texture::createTextureImage(std::vector<std::string> files, VkFormat Fmt) {
  ...
  for (int i = 0; i < imgs; i++) {
    pixels[i] = stbi_load(files[i].c_str(), &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
    if (!pixels[i]) {
      std::cout << "Not found: " << files[i] << "\n";
      throw std::runtime_error("failed to load texture image!");
    }
    std::cout << "[" << i << "]" << files[i] << " -> size: " << texWidth << "x" << texHeight
              << ", ch: " << texChannels << "\n";
    ...
    if (i == 0) {
      ...
      if ((curWidth != texWidth) || (curHeight != texHeight) || (curChannels != texChannels)) {
        throw std::runtime_error("multi texture images must be all of the same size!");
      }
    }
  }
  createTexture(texWidth, texHeight, 4, 1, pixels, Fmt);
  for (int i = 0; i < imgs; i++) {
    stbi_image_free(pixels[i]);
  }
}

void Texture::createTexture(int texWidth, int texHeight, int texChannels, int texChannelSize,
                            std::vector<void *> pixels, VkFormat Fmt) {

  VkDeviceSize imageSize = texWidth * texHeight * texChannels * texChannelSize;
  VkDeviceSize totalImageSize = imageSize * imgs;

  // std::cout << "Size: " << totalImageSize << "\n";
  ...
  BP->transitionImageLayout(textureImage, Fmt, VK_IMAGE_LAYOUT_UNDEFINED,
                            VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels, imgs);
  BP->copyBufferToImage(stagingBuffer, textureImage, static_cast<uint32_t>(texWidth),
                        static_cast<uint32_t>(texHeight), imgs);

  BP->generateMipmaps(textureImage, Fmt, texWidth, texHeight, mipLevels, imgs);
...
void Texture::createTextureImageView(VkFormat Fmt) {
  textureImageView =
      BP->createImageView(textureImage, Fmt, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels,
                          imgs == 6 ? VK_IMAGE_VIEW_TYPE_CUBE : VK_IMAGE_VIEW_TYPE_2D, imgs);
}
...
void Texture::initPixels(BaseProject *bp, int texWidth, int texHeight, int texChannels,
                         int texChannelSize, std::vector<void *> pixels, VkFormat Fmt,
                         bool initSampler) {
  BP = bp;
  imgs = pixels.size();
  // std::cout << "Creating texture\n";
  createTexture(texWidth, texHeight, texChannels, texChannelSize, pixels, Fmt);
...
VkDescriptorImageInfo Texture::getViewAndSampler() {
  return {sampler->getSampler(), textureImageView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
}
...
void Texture::setSampler(TextureSampler *smp) {
  sampler = smp;
}

/* SI DEVE AGGIUNGERE UNA VARIABILE per initSampler, ed inizializzarlo nella chiamata
 * createResources */
...
void FrameBufferAttachment::createResources() {
  ...
  BP->createImage(RP->width, RP->height, 1, 1, samples, format, VK_IMAGE_TILING_OPTIMAL, usage, 0,
                  VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, image, mem);
  view = BP->createImageView(image, format, aspect, 1, VK_IMAGE_VIEW_TYPE_2D, 1);

  if (doDepthTransition) {
    BP->transitionImageLayout(image, format, VK_IMAGE_LAYOUT_UNDEFINED,
...
VkDescriptorImageInfo FrameBufferAttachment::getViewAndSampler() {
  return {sampler->getSampler(), view, properties->finalLayout};
}

void FrameBufferAttachment::cleanup() {
  BaseProject *BP = RP->BP;

  // std::cout << "Cleaning up render pass attchment " << properties->swapChain << " " <<
  // properties->type << " " << properties->usage << "\n";
...
void RenderPass::init(BaseProject *bp, int w, int h, int _count,
                      std::vector<AttachmentProperties> *p, std::vector<VkSubpassDependency> *d,
                      bool initSampler) {
  BP = bp;
...
void RenderPass::createRenderPass() {
  ...
  // Build a contiguous array of color attachment references
  std::vector<VkAttachmentReference> colorRefs;
  colorRefs.reserve(colorAttchementsCount);
  for (int j = 0; j < attachments.size(); j++) {
    if (attachments[j].properties->type == COLOR_AT) {
      colorRefs.push_back(attachments[j].ref);
    }
  }
  ...
  VkResult result = vkCreateRenderPass(BP->device, &renderPassInfo, nullptr, &renderPass);
  if (result != VK_SUCCESS) {
    PrintVkError(result);
...
void RenderPass::begin(VkCommandBuffer commandBuffer, int currentImage) {
  ...
  bool connectedToSwapchain = false;

  for (int i = 0; i < properties.size(); i++) {
    clearValues[i] = properties[i].clearValue;
    if (properties[i].swapChain)
      connectedToSwapchain = true;
  }
  ...
  if (connectedToSwapchain) {
    renderPassInfo.renderArea.extent = {
        (uint32_t)(width < BP->swapChainExtent.width ? width : BP->swapChainExtent.width),
        (uint32_t)(height < BP->swapChainExtent.height ? height : BP->swapChainExtent.height)};
  } else {
...
std::vector<AttachmentProperties> *
RenderPass::getStandardAttchmentsProperties(StockAttchmentsConfiguration cfg, BaseProject *BP) {
...
RenderPass::getStandardDependencies(StockAttchmentsDependencies cfg) {
...
void Pipeline::init(BaseProject *bp, VertexDescriptor *vd, const std::string &VertShader,
                    const std::string &FragShader, std::vector<DescriptorSetLayout *> d,
                    std::vector<VkPushConstantRange> pk) {
  BP = bp;
  VD = vd;
  ...
  std::cout << "Vertex shader <" << VertShader << "> len: " << vertShaderCode.size() << "\n";
  std::cout << "Fragment shader <" << FragShader << "> len: " << fragShaderCode.size() << "\n";
...
void Pipeline::create(RenderPass *RP) {
  ...
  // Replicates one attachment state per color attachment in the subpass
  std::vector<VkPipelineColorBlendAttachmentState> blendAttachments;
  const uint32_t colorAttachmentCount = static_cast<uint32_t>(RP->colorAttchementsCount);
  if (colorAttachmentCount > 0) {
    blendAttachments.resize(colorAttachmentCount, colorBlendAttachment);
  }
  ...
  colorBlending.pAttachments = (colorAttachmentCount > 0) ? blendAttachments.data() : nullptr;
  ...
  VkResult result =
      vkCreatePipelineLayout(BP->device, &pipelineLayoutInfo, nullptr, &pipelineLayout);
  if (result != VK_SUCCESS) {
    PrintVkError(result);
  ...
  result = vkCreateGraphicsPipelines(BP->device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr,
                                     &graphicsPipeline);
  if (result != VK_SUCCESS) {
    PrintVkError(result);
...
VkShaderModule Pipeline::createShaderModule(const std::vector<char> &code) {
  ...
  VkResult result = vkCreateShaderModule(BP->device, &createInfo, nullptr, &shaderModule);
  if (result != VK_SUCCESS) {
    PrintVkError(result);
...
void DescriptorSetLayout::init(BaseProject *bp, std::vector<DescriptorSetLayoutBinding> B) {
  ...
  for (int i = 0; i < B.size(); i++) {
    ...
    if (((B[i].type == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER) ||
         (B[i].type == VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE) ||
         (B[i].type == VK_DESCRIPTOR_TYPE_SAMPLER)) &&
        (B[i].linkSize + B[i].count > imgInfoSize)) {
  ...
  VkResult result =
      vkCreateDescriptorSetLayout(BP->device, &layoutInfo, nullptr, &descriptorSetLayout);
  if (result != VK_SUCCESS) {
    PrintVkError(result);
...
void DescriptorSet::init(BaseProject *bp, DescriptorSetLayout *DSL,
  ...
  // std::cout << "pre resize " << uniformBuffers.size() << " " << size << "\n";
  uniformBuffers.resize(size);
  // std::cout << "uniformBuffers resized\n";
  uniformBuffersMemory.resize(size);
  // std::cout << "uniformBuffersMemory resized\n";
  toFree.resize(size);
  ...
  for (size_t i = 0; i < BP->swapChainImages.size(); i++) {
    ...
    for (int j = 0; j < size; j++) {
      ...
      descriptorWrites[j].descriptorType = DSL->Bindings[j].type;
      ...
      if (DSL->Bindings[j].type == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER) {
        ...
        descriptorWrites[j].pBufferInfo = &bufferInfo[j];
                 ...
                 (DSL->Bindings[j].type == VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE) ||
                 (DSL->Bindings[j].type == VK_DESCRIPTOR_TYPE_SAMPLER)) {
        // std::cout << "Writing combined image sampler " << j << ", count " <<
        // DSL->Bindings[j].count << ", link " << DSL->Bindings[j].linkSize << "\n";
        for (int k = 0; k < DSL->Bindings[j].count; k++) {
          ...
          imageInfo[h] = VaS;
        /*					imageInfo[h].imageLayout =
           VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL; imageInfo[h].imageView =
           VaS.textureImageView; imageInfo[h].sampler = VaS.textureSampler;
        */				}
    ...
    vkUpdateDescriptorSets(BP->device, static_cast<uint32_t>(descriptorWrites.size()),
...
void DescriptorSet::bind(VkCommandBuffer commandBuffer, Pipeline &P, int setId, int currentImage) {
  // std::cout << "DS[ci]: " << &descriptorSets[currentImage] << "\n";
  vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, P.pipelineLayout, setId,
